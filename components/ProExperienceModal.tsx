...
import type { ProExperience } from "@/data/pro_experiences";
...
export default function ExperienceModal({
  ...
  const nextImage = () => {
    if (experience && experience.gallery) {
      setCurrentImageIndex(
        (prev) => (prev + 1) % (experience.gallery?.length || 1)
      );
    }
  };
  ...
  const prevImage = () => {
    if (experience && experience.gallery && experience.gallery.length > 0) {
      setCurrentImageIndex(
        (prev) =>
          (prev - 1 + experience.gallery!.length) % experience.gallery!.length
      );
    }
  };
                  ...
                  {experience.links.map((link, idx) => (
                    ...
                    >
                      ...
                      <div className="flex-grow">
                        <h4 className="font-medium text-indigo-800 group-hover:text-indigo-900">
                          {link.title}
                        </h4>
