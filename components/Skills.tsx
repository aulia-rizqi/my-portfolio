              ...
              <div className={`bg-gradient-to-br ${category.gradient} rounded-3xl p-8 shadow-lg h-full`}>
                <div className="flex items-center gap-3 mb-6">
                  <div className="w-12 h-12 bg-gradient-to-r from-blue-400 to-indigo-500 rounded-xl flex items-center justify-center">
                    {category.icon === "heart" && <Heart className="h-6 w-6 text-white" />}
                    {category.icon === "code" && <Code className="h-6 w-6 text-white" />}
                  </div>
                  ...
                  {category.skills.map((skill, index) => {
                    ...
                    return (
                      <div key={index} className="flex items-start gap-4 p-3 bg-white/60 rounded-2xl">
                        <div
                          className={`w-10 h-10 bg-${skill.color}-100 rounded-xl flex items-center justify-center flex-shrink-0`}
                        >
                          <IconComponent className={`h-5 w-5 text-${skill.color}-600`} />
                        </div>
                        <div>
